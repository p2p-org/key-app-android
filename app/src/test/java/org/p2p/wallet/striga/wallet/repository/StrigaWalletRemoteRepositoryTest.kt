...
class StrigaWalletRemoteRepositoryTest {
    ...
    fun `GIVEN enrich account response for EUR WHEN getFiatAccountDetails THEN check response is parsed ok`() =
        runTest {
            ...
            assertEquals(StrigaFiatAccountStatus.ACTIVE, result.status)
            assertEquals("EUR09124356875233", result.internalAccountId)
            assertEquals("GB", result.bankCountry)
            assertEquals("The Bower, 207-211 Old Street, London, England, EC1V 9NR", result.bankAddress)
            assertEquals("GB30SEOU19870010116943", result.iban)
            assertEquals("SEOUGB21", result.bic)
            assertEquals("010116943", result.accountNumber)
            assertEquals("Simulator Bank", result.bankName)
            assertEquals("LORD VOLDEMORT", result.bankAccountHolderName)
            assertEquals("SIMULATOR", result.provider)
            assertEquals(null, result.paymentType)
            assertEquals(false, result.isDomesticAccount)
            assertEquals(emptyList<String>(), result.routingCodeEntries)
            assertEquals(null, result.payInReference)
        }
  ...
  "wallets": [
    {
      ...
      "total": 1
        ...
        // WHEN
        val actualResult: StrigaUserWallet = repository.getUserWallet()
    ...
    fun `GIVEN 400 response WHEN get wallets THEN user wallet is not gotten`() = runTest {
        ...
        // WHEN
        val actualResult = runCatching { repository.getUserWallet() }
        actualResult.assertThat().matchesPredicate {
            it.isFailure && it.exceptionOrNull() is HttpException
        }
    }
}
