...
class TokenDetailsFragment :
    ...
    private fun onTransactionClicked(transaction: Transaction) {
        when (transaction) {
            is Transaction.Swap -> addFragment(SwapTransactionFragment.create(transaction))
            is Transaction.Transfer -> addFragment(TransferTransactionFragment.create(transaction))
            else -> {
