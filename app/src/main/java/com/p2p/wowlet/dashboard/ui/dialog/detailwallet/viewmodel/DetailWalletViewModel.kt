package com.p2p.wowlet.dashboard.ui.dialog.detailwallet.viewmodel
...
class DetailWalletViewModel(val detailWalletInteractor: DetailWalletInteractor) :
    ...
    private val _percentages by lazy { MutableLiveData<Double>() }
    val getPercentages: LiveData<Double> get() = _percentages
    ...
    fun getActivityList(publicKey: String, icon: String, tokenName: String, tokenSymbol: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val detailList =
                detailWalletInteractor.getActivityList(publicKey, icon, tokenName, tokenSymbol)
            when (detailList) {
...
//    }
    ...
    fun getPercentages(walletItem: WalletItem) {
        viewModelScope.launch {
            withContext(Dispatchers.Main) {
                val percentages = detailWalletInteractor.getPercentages(walletItem)
                _percentages.value = percentages
            }
        }
    }
