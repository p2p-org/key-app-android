...
class TransferParsingStrategy(
    ...
    override suspend fun parseTransaction(
        ...
        val lamports: String = instructionInfo?.lamports
            ?.toLong()
            ?.toBigInteger()
            ?.toString()
            ?: instructionInfo?.amount
            ?: instructionInfo?.tokenAmount?.amount
            ?: ZERO_AMOUNT
        ...
        if (instruction?.programId != SystemProgram.PROGRAM_ID.toBase58()) {
            ...
            val initAccountInstruction =
                instructions?.firstOrNull { it.programId == TokenProgram.PROGRAM_ID.toBase58() }
