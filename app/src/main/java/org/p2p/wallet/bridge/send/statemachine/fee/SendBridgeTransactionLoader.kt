...
class SendBridgeTransactionLoader constructor(
    ...
    fun prepareTransaction(
        ...
        if (inputAmount != null) {
            ...
            emit(BridgeSendState.Static.ReadyToSend(token, fee, inputAmount, sendTransaction))
        } else {
            emit(BridgeSendState.Static.TokenZero(token, fee))
