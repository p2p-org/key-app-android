...
//        updateButtonState(newState)
    ...
    private fun handleCurrencySwitched(newState: CalculationEvent.CurrencySwitched) {
        ...
        newSendAnalytics.logSwitchCurrencyModeClicked(
            isCryptoMode = newState.isFiat,
            flow = NewSendAnalytics.AnalyticsSendFlow.SEND
        )
    }
...
//        view?.showUiKitSnackBar(resources.getString(R.string.send_reduced_amount_calculation_message))
    ...
    private fun handleFreeTransactionClicked() {
        newSendAnalytics.logFreeTransactionsClicked(NewSendAnalytics.AnalyticsSendFlow.SEND)
    ...
    override fun onTokenClicked() {
        newSendAnalytics.logTokenSelectionClicked(NewSendAnalytics.AnalyticsSendFlow.SEND)
...
//    }
    ...
    private fun logScreenOpened() {
        newSendAnalytics.logNewSendScreenOpened(NewSendAnalytics.AnalyticsSendFlow.SEND)
    }
}
