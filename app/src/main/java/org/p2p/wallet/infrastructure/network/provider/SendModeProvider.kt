...
class SendModeProvider(
    ...
    private fun getSendModeFromStorage(): CurrencyMode =
        try {
            val sendMode: SendMode = secureStorage.getObject(Key.KEY_SEND_MODE, SendMode::class)
                ?: SendMode(USD_READABLE_SYMBOL)
            if (sendMode.mode == USD_READABLE_SYMBOL) CurrencyMode.Usd
            else EMPTY_TOKEN
        } catch (e: Throwable) {
            Timber.tag(TAG).e(e)
            throw e
    ...
    private fun saveSendModeToStorage(value: CurrencyMode) {
        secureStorage.saveObject(
            Key.KEY_SEND_MODE,
            SendMode(if (value == CurrencyMode.Usd) USD_READABLE_SYMBOL else emptyString())
        )
    }
