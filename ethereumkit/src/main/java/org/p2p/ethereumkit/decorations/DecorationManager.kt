...
import org.p2p.ethereumkit.contracts.EmptyMethod
import org.p2p.ethereumkit.core.IEventDecorator
import org.p2p.ethereumkit.core.IMethodDecorator
import org.p2p.ethereumkit.core.ITransactionDecorator
import org.p2p.ethereumkit.core.ITransactionStorage
import org.p2p.ethereumkit.models.*
import java.math.BigInteger

class DecorationManager(private val userAddress: EthAddress, private val storage: ITransactionStorage) {
    ...
    fun decorateTransaction(from: EthAddress, transactionData: TransactionData): TransactionDecoration? {
    ...
    private fun decoration(from: EthAddress?, to: EthAddress?, value: BigInteger?, contractMethod: ContractMethod? = null, internalTransactions: List<InternalTransaction> = listOf(), eventInstances: List<ContractEventInstance> = listOf()): TransactionDecoration {
        for (decorator in transactionDecorators) {
