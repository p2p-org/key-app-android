...
import org.p2p.ethereumkit.spv.core.toInt
...
object ContractMethodHelper {
    ...
    fun encodedABI(methodId: ByteArray, arguments: List<Any>): ByteArray {
        ...
        arguments.forEach { argument ->
            when (argument) {
                ...
                is List<*> -> {
                    val addresses = argument.filterIsInstance<EthAddress>()
    ...
    private fun parseAddress(address: ByteArray): EthAddress {
        return EthAddress(address.copyOfRange(address.size - 20, address.size))
    }
    ...
    private fun parseAddressArray(positionStart: Int, inputArguments: ByteArray): List<EthAddress> {
        val sizePositionEnd = positionStart + 32
        val arraySize = inputArguments.copyOfRange(positionStart, sizePositionEnd).toInt()
        val addressArray = mutableListOf<EthAddress>()
    ...
    private fun encode(array: List<EthAddress>): ByteArray {
